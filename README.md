...
- [챌린지](#챌린지)
  ...
  - [4. 그림판에 그림 그리는 속도 저하 발생](#4-그림판에-그림-그리는-속도-저하-발생)
...
## 4. 그림판에 그림 그리는 속도 저하 발생
#### 문제점
그림판에 그림을 그릴수록 프레임 속도가 저하되는 현상이 발생하였습니다. 그림을 그리는 양이 적을 때는 문제가 없었지만 많은 양의 그림을 그릴수록 프레임 저하가 심해졌습니다..<br><br>

#### 원인
원인을 알아본 결과 `drawing` 함수가 그림을 그릴때 마다 `paths` 배열에 있는 모든 선들을 처음부터 그리고 있었습니다.
우선 `drawing` 함수가 그림을 그리는 과정을 설명하겠습니다. 
그림을 그리면 `paths` 상태에 선에 대한 정보가 담깁니다. 
...
pahts = [
  {
    "color": "#000000",
    "penWidth": 3, 
    "segments": ["M 309 96", "L 309 96", "L 309 96", ...]
  },
  {
    "color": "#000000",
    "penWidth": 5,
    "segments": ["M 128 30, "L 309 96", "L 309 96", ...]
  }
]
```

`paths`의 상태가 변경되면 `paths`를 의존성 배열에 갖고 있는 `drawing` 함수가 호출되며 `paths`의 모든 선들을 순회하며 그리게 됩니다. 이 과정에서 기존에 그렸던 선들까지 모두 그리는 연산을 수행하기 때문에 프레임 속도가 저하됐던 것입니다.

만약 50번째 선을 그린다면 기존 그렸던 50개의 그림을 연산을 수행했던 것입니다.<br><br>

#### 해결방안: `useMemo`를 활용한 최적화
...
이 최적화를 통해 의존성 배열이 변경되지 않은 경우 불필요한 계산 및 렌더링을 줄임으로써 성능을 저하를 줄였습니다.
...
하지만 오디오는 순서대로 재생이 되지만 `border` 스타일은 오디오 시간을 무시한채 빠르게 페이지마다 적용되고 끝나버렸습니다.
